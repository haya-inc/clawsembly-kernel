...
export function createOpenClawWizardRpcBridgeHarness(options: {
  ...
  return [
    ...
    "throw new Error('gateway reconnect timed out');",
    "const timeoutMs=Math.max(1,deadline-Date.now());",
    "try{return await client.request(method,params,{timeoutMs})}",
    "catch(error){",
    "const message=error instanceof Error?error.message:String(error);",
    "if(message!=='gateway not connected')throw error;",
    "ready=false;await sleep(50);",
    "}",
    "}",
    "};",
    ...
    "const listing=await requestGateway('agents.list',{});",
    ...
    "}else{",
    "result=await requestGateway(request.method,request.params);",
    "}",
